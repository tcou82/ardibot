...
%arduino_path%\hardware\tools\avr/bin/avrdude -C%arduino_path%\hardware\tools\avr/etc/avrdude.conf -v -p%arduino_firmware% -carduino -P%arduino_port% -b57600 -D -Uflash:w:%arduino_path%\tmp\build/app.ino.hex:i  > %arduino_path%\tmp\std.log 2> %arduino_path%\tmp\err.log
